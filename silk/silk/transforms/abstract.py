...
class ToNamedContext(Transform):
    ...
    def __init__(self, *names: List[Optional[str]], single_item: bool = False) -> None:
        ...
        # print("abstract __init__ names ", self._names)
    ...
    def __call__(self, item: Union[TupleType[Any], Any]) -> NamedContext:
        ...
        if not self._single_item:
            # print("not single item ", len(item), item[0].shape)
                #                        2       torch.Size([128, 416, 1])
                #  not single item       3       (376, 1241, 3)
            ensure_is_instance(item, tuple)
        else:
            item = (item,)

        # print("in abstract.py", type(item), len(item), self._names)
                            # <class 'tuple'>   2    ('image', None)
        #      in abstract.py <class 'tuple'>   3 ('image_1', 'image_2', None)
        # print(self._names, type(item[0]), type(item[1]), type(item[2]))
        #('image_1', 'image_2', None), ndarray, ndarray, list
...
class Compose(Transform):
    ...
    def __call__(self, item: Any, pose_flag=True) -> Any:
        ...
        # if pose_flag:
        #     item = self._transforms[0](item)
        #     item[:-1] = self._transforms[1](item[:-1])
...
class Map(Transform):
    ...
    def __call__(self, item: NamedContext) -> NamedContext:
        return item.map_only(["image_1", "image_2"], self._function, *self._args, **self._kwargs)
