...
class RandomCrop_getparam(Transform):
    def __init__(self, size, pad_if_needed) -> None:
        super().__init__()
        self.cropsize = size
        self.pad = pad_if_needed
        self.called = 0

    def __call__(self, image: torch.Tensor) : # return cropped image and a coord where the crop starts in original image
        img_sh = image.shape
        self.called += 1
        if self.called % 2 == 1:
            self.crop_indices = torchvision.transforms.RandomCrop.get_params(
                image, output_size=self.cropsize, )
        i, j, h, w = self.crop_indices
        # # randomcrop called!  25 858 164 164 torch.Size([3, 376, 1241])

        image = torchvision.transforms.functional.crop(image, i, j, h, w)
        return image, (i,j), img_sh
