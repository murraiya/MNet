...
class RandomHomographicSampler(Transform, HomographicSampler):
    ...
    def __init__(
        ...
        sampling_size: Tuple[int, int], # 3, 164, 164 ??
        rel_pose: torch.Tensor,
        sampling_mode: str = "bilinear",
        scaling_sampler: Union[Sampler, None] = None, # config to silk.config.sampler.Uniform
        ...
        HomographicSampler.__init__(self, batch_size, device, rel_pose)
    ...
    def _sample(
        self, images: torch.Tensor, direction="forward"
    ) -> torch.Tensor:
    ...
    def forward_sampling(self, images: torch.Tensor) -> torch.Tensor:
        ...
        torch.Tensor
            Generated crops.
        """
        return self._sample(images, "forward")
