...
class test_framework_KITTI_fomatted_odom(object):
    def __init__(self, root, train=True, sequence_length = 2):
        ...
        self.frame_list_path = self.root/'test.txt'        
    ...
    def generator(self):
        for sample in self.samples:
            ...
            yield {
                ...
                'abs_pose': sample["abs_pose"], 
                'extrinsic': sample["extrinsic"][0],
    ...
    def crawl_folders(self):
        ...
        for scene in tqdm(scenes): # scene: '/data/formatted_kitti_odom/00_02'
            ...
            poses[scene] = np.zeros((poses_tmp.shape[0], 4, 4)).astype(np.float64)
        ...
        for two_frame in tqdm(self.two_frames):
            ...
            relative_scene_pose = np.linalg.inv(poses[scene][frame_1_id]) @ poses[scene][frame_2_id]    
            relative_scene_pose = ( #clement pinard did this when dumping poses but deepFEPE did not do it. so im following FEPE
                extrinsic[scene]
                @ relative_scene_pose
